...
<html>
  <body>
    ...
    // Get the item name
    $itemNameResult = mysqlQuery("SELECT name FROM items WHERE items.id=$itemId", $link) or die("ERROR: Query failed");
    if (mysqli_num_rows($itemNameResult) == 0)
      $itemNameResult = mysqlQuery("SELECT name FROM old_items WHERE old_items.id=$itemId", $link) or die("ERROR: Query failed");
    if (mysqli_num_rows($itemNameResult) == 0)
    {
      commit($link);
    ...
    $bidsListResult = mysqlQuery("SELECT * FROM bids WHERE item_id=$itemId ORDER BY date DESC", $link) or die("ERROR: Bids list query failed");
	...
	// Get the bidder nickname	
    	if ($userId != 0)
	{
	  $userNameResult = mysqlQuery("SELECT nickname FROM users WHERE id=$userId", $link) or die("ERROR: User nickname query failed");
