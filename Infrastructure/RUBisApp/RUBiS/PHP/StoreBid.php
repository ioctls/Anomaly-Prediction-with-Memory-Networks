...
<html>
  <body>
    ...
    mysqlQuery("LOCK TABLES bids WRITE, items WRITE", $link) or die("ERROR: Failed to acquire locks on items and bids tables.");
    $result = mysqlQuery("SELECT max_bid FROM items WHERE id=$itemId", $link) or die("ERROR: Failed to update number of bids in database. DEADLOCK!!");
    $row = mysqli_fetch_array($result);
    if ($maxBid > $row["max_bid"])
      mysqlQuery("UPDATE items SET max_bid=$maxBid WHERE id=$itemId", $link) or die("ERROR: Failed to update maximum bid in database. DEADLOCK!!");

    mysqlQuery("INSERT INTO bids VALUES (NULL, $userId, $itemId, $qty, $bid, $maxBid, '$now')", $link) or die("ERROR: Failed to insert new bid in database. DEADLOCK!!");
    mysqlQuery("UPDATE items SET nb_of_bids=nb_of_bids+1 WHERE id=$itemId", $link) or die("ERROR: Failed to update number of bids in database. DEADLOCK!!");
    mysqlQuery("UNLOCK TABLES", $link) or die("ERROR: Failed to unlock items and bids tables.");
    commit($link);
