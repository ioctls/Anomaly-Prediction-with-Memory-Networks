...
class LSTMED(Algorithm, PyTorchUtils):
	...
	#changes
        threshold_loader = DataLoader(dataset=sequences, batch_size=1, drop_last=True,
                                  sampler=SubsetRandomSampler(indices), pin_memory=True)
        mvnormal = multivariate_normal(self.mean, self.cov, allow_singular=True)
        scores = []
        outputs = []
        errors = []
        for idx, ts in enumerate(threshold_loader):
        ...
        #self.lstmed.threshold = np.nanmax(scores)
        scores_mean = np.nanmean(scores)
        scores_sd = np.nanstd(scores)
        self.lstmed.threshold = scores_mean + 3*scores_sd
    ...
    def predict_eval(self, X: pd.DataFrame):
        X.interpolate(inplace=True)
        X.bfill(inplace=True)
        data = X.values
        sequences = [data[i:i + self.sequence_length] for i in range(data.shape[0] - self.sequence_length + 1)]
        data_loader = DataLoader(dataset=sequences, batch_size=1, shuffle=False, drop_last=False)
        ...
        for each in scores:
            eval_scores.append(np.max(each))
    ...
    def online_predict_train(self, X: pd.DataFrame):
        ol_train_seqs = [X[i:i + self.sequence_length] for i in range(X.shape[0] - self.sequence_length + 1)]
        X.interpolate(inplace=True)
        X.bfill(inplace=True)
        data = X.values
        sequences = [data[i:i + self.sequence_length] for i in range(data.shape[0] - self.sequence_length + 1)]
        ...
        for idx, ts in enumerate(data_loader):
            ...
            print("hello")
            #online training
            """loss = nn.MSELoss(size_average=False)(output, self.to_var(ts.float()))
            self.lstmed.zero_grad()
            loss.backward()
            optimizer.step()"""
            self.fit(ol_train_seqs[idx].copy())
